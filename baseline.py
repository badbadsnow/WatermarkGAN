...
from watermark_gan import BigVGAN_AudioSeal
...
def train(rank, a, h):
    ...
    if cp_g is None or cp_do is None:
        ...
        wmd.load_state_dict(state_dict_do["wmd"])
    ...
    if h.num_gpus > 1:
        ...
        wmd = DistributedDataParallel(wmd, device_ids=[rank]).to(device)

    optim_g = torch.optim.AdamW(
        generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2]
    )
    optim_d = torch.optim.AdamW(
        itertools.chain(mrd.parameters(), mpd.parameters(), wmd.parameters()),
        h.learning_rate,
    ...
    def validate(rank, a, h, loader, mode="seen"):
        ...
        with torch.no_grad():
            ...
            for j, batch in enumerate(tqdm(loader)):
                ...
                if hasattr(wmd, "module"):
                    detect_result, detect_message = wmd.module.detect_watermark(y_g_hat)
                else:
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # df判别结果 = 周期数*[torch.Size([1, 102]), torch.Size([1, 102]), torch.Size([1, 105]), torch.Size([1, 105]), torch.Size([1, 110])]
            # fmap特征图 = 周期数*conv层数*[torch.Size([1, 32, 1366, 2]), torch.Size([1, 128, 456, 2]), torch.Size([1, 512, 152, 2]), torch.Size([1, 1024, 51, 2]), torch.Size([1, 1024, 51, 2]), torch.Size([1, 1, 51, 2])]
            ...
            # MRD
            # ds判别结果 = 分辨率数*[torch.Size([1, 4617]), torch.Size([1, 5125]), torch.Size([1, 5397])]
            # fmap特征图 = 分辨率数*conv层数*[]
            ...
            if steps >= a.freeze_step:
                ...
                grad_norm_wmd = torch.nn.utils.clip_grad_norm_(
                    wmd.parameters(), clip_grad_norm
                )
                optim_d.step()
            else:
                print(
                ...
                if steps % a.checkpoint_interval == 0 and steps != 0:
                    ...
                    save_checkpoint(
                        checkpoint_path,
                        {
                            ...
                            "wmd": (wmd.module if h.num_gpus > 1 else wmd).state_dict(),
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["D:/dataset/LibriTTS", "D:/dataset/LibriTTS"],
    )
    parser.add_argument(
