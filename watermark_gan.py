...
class WatermarkEmbedder(nn.Module):
    def __init__(self, watermark_dim=512):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Conv1d(1, watermark_dim // 2, kernel_size=3, stride=1, padding=1),
            nn.GELU(),
            nn.Conv1d(watermark_dim // 2, watermark_dim, kernel_size=1, stride=1, padding=0),
        )
    ...
    def forward(self, x, watermark):
        ...
        watermark_reshaped = watermark.unsqueeze(1)  # [1, 1, 512]

        wm_signal = self.encoder(watermark_reshaped)

        if wm_signal.shape != x.shape:
            wm_signal = nn.functional.adaptive_avg_pool1d(wm_signal, x.size(-1))

        output = x + self.alpha * wm_signal
        return output


class BigVGAN_Watermark(torch.nn.Module, ):
    def __init__(self, h: AttrDict, use_cuda_kernel: bool = False):
        ...
        self.watermark_embedder = WatermarkEmbedder(watermark_dim=h.get("upsample_initial_channel", 512))
    ...
    def forward(self, x, watermark=None):
        ...
        if watermark is not None:
            x = self.watermark_embedder(x, watermark)
